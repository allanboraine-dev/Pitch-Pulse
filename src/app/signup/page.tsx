...
export default function PlayerSignupPage() {
  ...
  const [state, action, isPending] = useActionState(async (prevState: any, formData: FormData) => {
    ...
    redirect('/dashboard') // Or a "waiting for approval" page
  }, null)
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-900 p-4">
      <div className="max-w-md w-full bg-gray-800 rounded-2xl shadow-xl p-8 border border-gray-700">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-white mb-2 tracking-tight">Player Sign Up</h1>
          <p className="text-gray-400 text-sm">Join your club and get ready to play.</p>
        </div>
        ...
        <form action={action} className="space-y-5">
          ...
          <div>
            ...
            <select
              ...
              required
              disabled={loading}
              className="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-xl text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all outline-none disabled:opacity-50"
            >
              <option value="">{loading ? 'Loading clubs...' : 'Select your club...'}</option>
              {clubs.map(club => (
